...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
  	...
  	p = lambda k: np.exp(score[k]) / sum_exp
  	loss += (-np.log(p(y[i])))
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  each_p = np.exp(score) / sum_exp
  p = each_p[range(score.shape[0]),y]
  loss = np.mean(-np.log(p)) + 0.5*reg*np.sum(W*W)

  each_p[range(score.shape[0]),y] -=1
  dW = (X.T).dot(each_p) / num_train + reg*W
  #############################################################################
  #                          END OF YOUR CODE                                 #
  #############################################################################
